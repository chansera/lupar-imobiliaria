...
<main class="min-h-screen bg-white pb-20" in:fade>
	<div class="relative h-[50vh] w-full min-w-full bg-brand md:h-[65vh]">
		{#if capa}
			<enhanced:img
		...
		<div class="absolute bottom-0 left-0 w-full p-6 md:p-12">
			<div class="container mx-auto">
				<div class="max-w-3xl">
					<span class="mb-4 inline-block rounded-full bg-brand px-4 py-1 text-xs font-bold uppercase tracking-widest text-white">
		...
		<div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
			<div class="space-y-8 lg:col-span-2">
				...
				<div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-xl lg:hidden">
					...
					<p class="text-3xl font-black text-brand">{formatarMoeda(imovel.preco)}</p>
				</div>

				<section class="rounded-2xl border border-gray-100 bg-white p-8 shadow-sm">
					<h2 class="mb-6 text-2xl font-bold text-brand">Descrição Detalhada</h2>
				...
				{#if galeria.length > 0}
					<section class="space-y-6">
						<h2 class="text-2xl font-bold text-brand">Fotos</h2>
			...
			<div class="space-y-6">
				<div class="sticky top-24 hidden rounded-2xl border border-gray-100 bg-white p-8 shadow-2xl lg:block">
					...
					<p class="text-4xl font-black text-brand">{formatarMoeda(imovel.preco)}</p>

					<div class="mb-8 mt-8 space-y-4">
						<h3 class="border-b border-brand pb-2 text-sm font-bold uppercase text-brand-dark">Atributos</h3>
						<div class="grid grid-cols-2 gap-4">

							{#if imovel.tipo === 'Casa' || imovel.tipo === 'Apartamento'}
								<div class="rounded-lg bg-slate-50 p-3 text-center">
									<span class="block text-xl font-bold text-brand">{imovel.quartos}</span>
									<span class="text-xs uppercase text-gray-500">Quartos</span>
								</div>
								<div class="rounded-lg bg-slate-50 p-3 text-center">
									<span class="block text-xl font-bold text-brand">{imovel.banheiros}</span>
									<span class="text-xs uppercase text-gray-500">Banhos</span>
								</div>
								<div class="rounded-lg bg-slate-50 p-3 text-center">
									<span class="block text-xl font-bold text-brand">{imovel.areaConstruida}m²</span>
							...
							{#if 'areaTotal' in imovel && imovel.areaTotal}
								<div class="rounded-lg bg-slate-50 p-3 text-center">
									<span class="block text-xl font-bold text-brand">{imovel.areaTotal}m²</span>
                ...
                <a
                    ...
                    target="_blank"
                    class="hidden md:flex items-center gap-2 bg-purple-900 hover:bg-purple-800 text-white px-6 py-3 rounded-lg font-bold transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5"
                >
